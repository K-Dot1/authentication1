...
def authenticate_user(db, username: str, password: str) -> Optional[UserInDBSchema]:
    ...
    if db_user and authentication.verify_password(password, db_user.hashed_password):
...
def reset_password(reset_data: ResetPassword, db: Session = Depends(get_db)):
    ...
    db_user.hashed_password = authentication.hash_password(reset_data.new_password)
    db.commit()
